...
public class Blowfish {
    ...
    /**
     * Creates a new Blowfish object using the specified key (oversized key will be cut).
     *
     * @param key the password (treated as a real unicode array)
     */
    public Blowfish(String key) {
        // hash down the key to a 160bit key
    ...
    /**
     * Encrypts a string (treated in UNICODE) using the standard Java random generator,
     * which isn't that great for creating IVs
     *
     * @param sPlainText string to be encrypted
     * @return encrypted string in binHex format
     */
    public String encryptString(String sPlainText) {
        // get the IV
        long lCBC_IV;
        synchronized (m_rndGen) { lCBC_IV = m_rndGen.nextLong(); }

        // map the call;
        return encStr(sPlainText, lCBC_IV);
    }

    private String encStr(String sPlainText, long lNewCBC_IV) {
        ...
        byte[] newCBC = new byte[BlowfishCBC.BLOCKSIZE];
        longToByteArray(lNewCBC_IV, newCBC, 0);

        return bytesToBinHex(newCBC, BlowfishCBC.BLOCKSIZE) +
                bytesToBinHex(buf, buf.length);
    }

    /**
     * decrypts a hexBin string (handling is case-sensitive)
     * @param sCipherText hexBin string to be decrypted
    ...
    public String decryptString(String sCipherText) {
        ...
        // get the CBC IV
        byte[] CBC_IV = new byte[BlowfishCBC.BLOCKSIZE];
        int nNumOfBytes = binHexToBytes(sCipherText, CBC_IV, 0, 0, BlowfishCBC.BLOCKSIZE);
    ...
    private static class BlowfishCBC extends BlowfishECB {
        ...
        public void setCBC(long lNewCBC)
        {
            m_lCBC = lNewCBC;
        }

        public void setCBC(byte[] newCBC)
        {
            m_lCBC = byteArrayToLong(newCBC, 0);
        }
     ...
     * @return the binHex string
     */
    private static String bytesToBinHex(byte[] data, int nNumOfBytes) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.setLength(nNumOfBytes << 1);

        int nPos = 0;
        for (int nI = 0; nI < nNumOfBytes; nI++) {
            stringBuilder.setCharAt(nPos++, HEXTAB[(data[nI] >> 4) & 0x0f]);
            stringBuilder.setCharAt(nPos++, HEXTAB[data[nI] & 0x0f]);
        }
